      ...
      <Wrapper>
        <div className="bg-white font-sans text-sm pb-4 border-b border-slate-200 mb-4">
           <div className="flex gap-2 p-3">
              <div className={`w-10 h-10 rounded overflow-hidden flex items-center justify-center shrink-0 ${hasLogo ? 'bg-white border border-slate-100' : 'bg-slate-200'}`}>
                 {hasLogo ? <img src={clientSettings.logoUrl} className="w-full h-full object-contain" alt="Client Logo" /> : null}
              </div>
              <div>
                 <div className="font-semibold text-xs text-slate-900">{post.client || 'Your Name'}</div>
    ...
    <Wrapper>
        <div className="px-4 pb-4 font-sans">
           <div className="flex items-center gap-3 mb-4">
              <div className="w-10 h-10 rounded-full flex items-center justify-center text-white overflow-hidden shrink-0 border border-slate-100" style={{ backgroundColor: hasLogo ? '#fff' : brandColor }}>
                 {hasLogo ? <img src={clientSettings.logoUrl} className="w-full h-full object-contain" alt="Client Logo" /> : <MapPin size={20} />}
              </div>
              <div>
                 <div className="font-bold text-slate-900 text-sm">{post.client || 'Your Business'}</div>
