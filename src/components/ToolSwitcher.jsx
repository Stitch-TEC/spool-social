...
const ToolSwitcher = () => {
  ...
  useEffect(() => {
    ...
    const onKey = (e) => {
      ...
      setOpen(false);
      // Do not steal focus if the user has already moved outside the launcher.
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ...
        className="min-h-11 min-w-11 flex items-center justify-center text-slate-600 hover:bg-slate-100 rounded-lg focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-600"
      >
        <LayoutGrid size={20} />
      </button>
      {open && (
        <div id={panelId} className="absolute left-0 mt-2 w-64 max-w-[calc(100vw-2rem)] max-h-[calc(100dvh-5rem)] overflow-y-auto bg-white border border-slate-200 rounded-xl shadow-lg p-1 z-50">
          <div className="px-3 py-2 text-[10px] font-bold uppercase tracking-widest text-slate-600">Stitch Suite</div>
          {STITCH_APPS.map(app => {
            ...
            return app.status === 'soon' ? (
              <div key={app.id} className="flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm text-slate-600 cursor-default">
              ...
              <a
                ...
                className={`min-h-11 flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm hover:bg-slate-50 focus-visible:outline-2 focus-visible:-outline-offset-2 focus-visible:outline-indigo-600 ${current ? 'text-indigo-700 font-bold' : 'text-slate-700 font-medium'}`}
              >
                {tile}
                <span className="flex-1 min-w-0 truncate">{app.name} <span className="font-normal text-slate-600">· {app.tagline}</span></span>
